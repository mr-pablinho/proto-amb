...
class AuditLogger:
    def __init__(self, filename="audit_detailed_log.csv"):
        ...
        self.headers = [
            ...
            "Router_Model", "Router_Input_Tokens", "Router_Output_Tokens", "Router_Cost", "Router_Files",
    ...
    def calculate_cost(self, model_name, input_tok, output_tok):
        """Calculates cost based on model type."""
        # Normalize tokens to Millions for pricing math
        ...
        return round(cost, 6) # Precision to 6 decimals (e.g., $0.000123)

    def log_requirement(self, req_id, req_text, 
                        router_data, auditor_data):
        """
        router_data = {'model': str, 'input': int, 'output': int, 'files': str}
        auditor_data = {'model': str, 'input': int, 'output': int, 'status': str, 'reasoning': str}
        """
